...
#      model = load_model("./models/resv152.h5")
...
# Function for loading the pre-trained network and predicting
def build(imagepath, model):
    try:
        # Load the pre-trained model
        print("[Info] Loading pre-trained network...")
     #    model = load_model("./models/resv152.h5")

        # Mapping output classes
        output_dict = {
            'Apple_scab': 0,
            'Apple_black_rot': 1,
            'Apple_cedar_apple_rust': 2,
        ...
        img = cv2.imread(imagepath)
        if img is None:
            raise ValueError(f"Unable to load image at {imagepath}. Please check the file path.")
        ...
        print(f"[Error] {str(e)}")
        return "Error in processing the image or predicting the result."
